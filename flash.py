...
import random
APP_ID =20872837
APP_HASH='f3e4b12f39e489ce895c18102732b54a'
#bot_token='6944694546:AAECGUEoKgAIMLQSFRQzQaFBhWzqbQx7cC8'
...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@FLS_44'))
	except BaseException:
		pass		
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@FLS_45'))
	except BaseException :
		pass
@abbas.on(events.NewMessage)
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@FLS_46'))
	except BaseException :
		pass
...
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@FLS_47'))
	except BaseException :
		pass
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinu = await abbas(JoinChannelRequest('FLS_44'))
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await flash1.send_message(event.chat_id, f"تم الانتهاء من التجميع | SH")
                break
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               joinu = await abbas(JoinChannelRequest('FLS_44'))
               channel_entity = await abbas.get_entity(pot)
               await abbas.send_message(pot, '**جاري بدأ عملية التجميع بواسطة فلاش**')
               await abbas.send_message(pot, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('FLS_44'))
...
async def Abbas(event):
    if event.message.message.startswith("تمويل") and event.sender_id in Devflash:
